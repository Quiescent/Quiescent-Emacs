                                    ...
                                    (if (not (eq char (quiescent-closing-bracket-for (car brackets))))
                                        (progn
                                          (dolist (bracket brackets)
                                            (push (quiescent-closing-bracket-for bracket) result))
...
** Icomplete Vertical
I used i-complete a while ago and it's a great package but it lacked
some attention.  Now it has a vertical mode and better support for
matching.  I'm going to try out the fuzzy version of vertical
iComplete: ~fido-vertical-mode~.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref fido-mode :lexical t
  (fido-vertical-mode t)

  (defun quiescent-icomplete-up-directory ()
    "Delete a directory backwards from the end of the prompt."
    (interactive)
    (save-excursion)
    (goto-char (point-max))
    (backward-char)
  ...
  (define-key icomplete-fido-mode-map (kbd "C-^") #'quiescent-icomplete-up-directory)
...
*** Org Clock
There's a built-in time tracking facility in org mode.  I use it to
bill clients.  Unfortunately, it doesn't provide an out-of-the-box
solution to daily billing.  I'm going to take a look at snippets from
the web for generating reports that I'm happy with.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref org-rangereport :lexical t
#+end_src

*** Org Archive Done
Sometimes I have a lot of DONE items clogging up my agenda and making
things slower.  When that happens, it's great to be able to archive
all the done items.  This snippet from SO
(https://stackoverflow.com/questions/6997387/how-to-archive-all-the-done-tasks-using-a-single-command)
does the trick.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref org-archive-subtrees :lexical t
...
** Coffee
I roast coffee.  I've been selling it to my family at a little more
than cost because it saves them money and they like it.  I use
~org-mode~ to keep track of the orders so I thought that it might be a
good idea to write some code to automate capturing orders etc.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref coffee :exports none :lexical t
...
** Fixed Pitch Mode
Emacs has support for variable-pitch font display.  I've never used
it, but writers seem to think that it helps them, so I'm going to give
it a bash.  There's a package that takes care of most of the details,
I'm going to give it a bash.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref mixed-pitch :lexical t
...
I often find myself switching to Firefox to look up a word.  That can
be slow and cumbersome, so I'm looking into Emacs packages to replace
that process.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref define-word :lexical t
...
Since I wrote this, Nicolas Rougier (an actual designer) created Nano
Emacs.  It's elegant and it's based on actual design principles.  I
loaded it up to test it out and it made my previous Emacs setup look
dated.

Why is Nano's appearance so much better?  I think that Nano looks
better because it optimises for comprehension of the data on the page
rather than cramming things in or making things look flashy.  It
displays what's necessary using modern typography practices and
displays things in a more readable way.  I'm going to switch to it and
see how things go.
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref themes :lexical t
  (progn
    ...
    ;; Reload custom because nano overwrote stuff that I want from there.
...
** Ansi-Coloured compilation
Some compilers output colourful text.  This is done with ansi control
characters.  Unfortunately, the compilation buffer doesn't interpret
those characters.  When I run into compilers that spit those
characters out, I use this function (based on a snippet from Malabarba
https://endlessparentheses.com/ansi-colors-in-the-compilation-buffer-output.html)
to interpret ansi control characters in compilation buffers in the
current session.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref ansi-coloured-compilation :lexical t
