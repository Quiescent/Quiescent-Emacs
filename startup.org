...
** TRAMP
Tramp is a fantastic system which uses UNIX tools to make any machine
or environment appear as though it were local to this machine.  There
are aspects of it that require customisation.  For instance when it
opens compressed files then it indicates progress and shadows the
eldoc prompt.

#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref inhibit-tramp-progress :lexical t
