...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref helm :lexical t
  ...
  (use-package helm
      ...
      (helm-grep-git-grep-command            "git --no-pager grep -n%cH --color=always --full-name -e %p -- %f ':!*package-lock.json'")
      :config (progn
                (require 'helm-config)
                (helm-mode 1)
...
#+begin_src emacs-lisp :tangle no :noweb yes :noweb-ref js2-raise-variable :lexical t
  (defun quiescent-js2-raise-variable (p)
    ...
    (let* ((node       (js2r--closest-extractable-node))
           (expression (buffer-substring (js2-node-abs-pos node) (js2-node-abs-end node))))
      (backward-up-list)
      (let ((new-node (js2r--closest-extractable-node)))
        (delete-region (js2-node-abs-pos new-node) (js2-node-abs-end new-node)))
      (insert expression)))
