...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref company-mode :lexical t
  (defun quiescent-company-with-dropdown ()
    "Switch to completing with a dropdown."
    (interactive)
    (setq company-frontends '(company-pseudo-tooltip-frontend)))

  (defun quiescent-set-company-backends ()
    "Set the company back ends to preview only."
  ...
  (use-package company
   ...
   (progn
     ...
     (define-key company-active-map [down]       #'quiescent-company-with-dropdown)
     (define-key company-active-map [up]         #'quiescent-company-with-dropdown)
     ...
     (setq company-frontends '(company-preview-frontend))
     (advice-add #'company-complete :before #'quiescent-set-company-backends)
     (advice-add #'company-auto-begin :before #'quiescent-set-company-backends))
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref eacl :lexical t
  (use-package eacl
    :ensure t
    :bind (("s-TAB" . eacl-complete-multiline)
           ("M-s-i" . eacl-complete-line)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref hide-show :lexical t
  ...
  (use-package hideshow
      ...
      :hook ((c-mode-common emacs-lisp-mode
           ...
           sgml-mode html-mode) 
         . quiescent-activate-hs-minor-mode))
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref paredit :lexical t
  ...
  (use-package paredit
    ...
    :hook ((emacs-lisp-mode eval-expression-minibuffer-setup
         ...
         slime-repl-mode)
        . quiescent-activate-paredit-mode))
#+END_SRC
** Electric Pair Mode
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref emacs-refactor :lexical t
  ...
  (use-package emr
   ...
   :hook (prog-mode . quiescent-emr-initialize))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref helm :lexical t
  ...
  (use-package helm
      ...
      :bind (("M-y"       . helm-show-kill-ring)
             ...
             ("M-x"       . helm-M-x)
             ("C-S-f"     . quiescent-helm-grep-do-git-grep)
             ("C-c p s g" . quiescent-helm-grep-do-git-grep)
             ("C-h C-f"   . helm-recentf))
      :config (progn
                (require 'helm-config)
                (helm-mode 1)
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref web-mode :lexical t
  (use-package web-mode
      ...
      :hook (web-mode . quiescent-set-company-to-dabbrev-only))

  (defun quiescent-set-company-to-dabbrev-only ()
    "Set the company backends to dabbrev only."
    (when (null quiescent-starting-up)
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref tide :lexical t
  (use-package tide
   :ensure t
   :hook ((js2-mode . quiescent-setup-tide-mode)
       (js2-jsx-mode . quiescent-setup-tide-mode)
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref slime :lexical t
  ...
  (use-package slime
    ...
    (progn
      ...
      (define-key slime-repl-mode-map (kbd "C-x C-s") #'slime-repl-save-history)
      (define-key slime-repl-mode-map (kbd "C-x C-f") #'slime-repl-load-history)
    ...
    :init (progn (setq inferior-lisp-program "sbcl")
                 ...
                 (slime-setup '(slime-fancy slime-company slime-c-p-c slime-highlight-edits)))
    :hook ((slime-mode . quiescent-setup-slime-completion)
           (slime-repl-mode . quiescent-setup-slime-completion)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref cider :lexical t
  ...
  (use-package cider
    :ensure t
    :hook ((clojure-mode . quiescent-setup-clojure-hooks)
           (cider-repl-mode . quiescent-setup-cider-repl-hooks))
    :config
    (progn
          ...
          (setq dired-filter-group-saved-groups
                ...
                ("Source Code"
                 (extension "el" "scala" "rs" "asm"
                         ...
                         "hi" "hs" "py" "java"
                         "sh" "asd"))))))
      :hook (dired-mode . quiescent-dired-filter-group-mode))
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref ispell :lexical t
  ...
  (use-package flyspell
    ...
    :hook ((text-mode bibtex-mode LaTeX-mode TeX-mode) . quiescent-turn-on-flyspell))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref prism :lexical t
  ...
  (use-package prism
    ...
    ;; original doom theme
    :config (prism-set-colors :num 24
    ...
    :hook (((clojure-mode
           ...
           ((python-mode makefile-mode haskell-mode yaml-mode)
