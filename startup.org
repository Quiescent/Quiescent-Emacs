...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref flycheck :lexical t
  ...
  (with-eval-after-load "flycheck-mode"
    (progn
      ...
      (define-key flycheck-mode-map (kbd "M-n") #'flycheck-previous-error)))
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref semantic :lexical t
  ...
  (setq-default semanticdb-find-default-throttle '(file))
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref rust :lexical t
  ...
  (defun quiescent-setup-rust-flycheck ()
    "Setup flycheck in Rust mode."
    (when (null quiescent-starting-up)
      (flycheck-rust-setup 1)))

  (use-package flycheck-rust
      :ensure t
      :hook (rust-mode . quiescent-setup-rust-flycheck))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref slime :lexical t
  ...
  (use-package slime
      :ensure t
      :config (define-key slime-mode-map (kbd "s-q") #'quiescent-close-slime-help)
...
The built in font-lock for Common Lisp is lacking in a few ways.
There are a couple of built in functions, macros and variables which
it doesn't highlight (apparently).  The author of ~stumpwm~, the
window manager written in Common Lisp, wrote some extra rules for
~font-lock~.

#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref cl-font-lock :lexical t
...
#+BEGIN_SRC emacs-lisp :tangle no :noweb yes :noweb-ref load-org :lexical t
  ...
  (setq org-capture-templates
        (quote (("t" "Task" entry (file "~/work.org")
                 "* TODO %? \n%U\n%a\n")
